...
if(empty($category)){
    $this->title = Yii::$app->feehi->website_title.' | 重庆万达文旅城|重庆万达城|重庆融创万达文旅城';
}else{
    $this->title =empty($category->seo_title) ? $category->name:$category->seo_title;
    $this->registerMetaTag(['name' => 'keywords', 'content' => $category->seo_keywords], 'keywords');
